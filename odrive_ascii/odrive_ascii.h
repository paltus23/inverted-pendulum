...
extern "C"
{
    ...
    // read_fn must block (up to timeout_ms) until at least one byte is available or
    // timeout. It may return 0 on timeout.
    typedef size_t (*odrv_write_fn)(const uint8_t *buf, size_t len);
    ...
    // Utilities
    uint8_t
    odrv_ascii_checksum_xor(const char *line_without_star); // xor of all chars
    // Send one formatted command (printf-style for the command portion), optionally
    // with checksum, then read a single CRLF-terminated line into out (stripped of
    // CRLF). Returns >=0 length, or <0 on error.
    ...
    // Parameter write: w <property> <value> -> returns reply (often empty; may be
    // "ok" if FW does so)
